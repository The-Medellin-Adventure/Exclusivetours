...
(function(window){
  var tryCount = 0;
  var maxTries = 150;
  function readyCheck(){ tryCount++; if (window.Marzipano && (window.APP_DATA !== undefined)) { try { if (typeof window.__main === 'function') window.__main(); else console.error('No __main function found.'); } catch(e){ console.error('Error running main:', e); } } else { if (tryCount < maxTries) setTimeout(readyCheck, 200); else console.warn('Marzipano or APP_DATA not available after wait'); } }
