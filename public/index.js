...
(function(){
  if (typeof atob === 'undefined') {
    globalThis.atob = function(b) {
      try { return window.atob(b); }
      catch(e) { return (typeof Buffer !== 'undefined') ? Buffer.from(b,'base64').toString('binary') : b; }
    };
  }
})();
...
(function(global){
  try {
    ...
    var O = {};
    ['log','warn','error','info','debug'].forEach(function(k){ O[k]=C[k]||function(){}; });
    Object.defineProperty(global,'console',{ configurable:false, get:function(){ return { log:O.log, warn:O.warn, error:O.error, info:O.info, debug:O.debug, clear:function(){}, table:function(){} }; } });
  } catch(e){ }
})(typeof window!=='undefined'?window:this);

/* Runner: waits until Marzipano and APP_DATA are available, then calls window.__main() */
(function(window){
  var tryCount = 0;
  var maxTries = 150;
  function readyCheck(){ tryCount++; 
    // detect APP_DATA either as window.APP_DATA or as global APP_DATA
    var appData = window.APP_DATA || (typeof APP_DATA !== 'undefined' ? APP_DATA : undefined);
    if (window.Marzipano && appData) { 
      try { if (typeof window.__main === 'function') window.__main(); else console.error('No __main function found.'); } catch(e){ console.error('Error running main:', e); }
    } else { if (tryCount < maxTries) setTimeout(readyCheck, 200); else console.warn('Marzipano or APP_DATA not available after wait'); }
  }
